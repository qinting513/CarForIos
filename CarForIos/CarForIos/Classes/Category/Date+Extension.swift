...
extension Date {
    ...
    func toAgoString(_ useRelativeDay:Bool = false, agoInDigit:Bool = true) -> String {
        let curDate = Date()
        //print("self=\(self), curDate=\(curDate)")
        //self=2016-06-28 07:07:36 +0000, curDate=2016-06-29 03:27:23 +0000
        var components = self.toDateComponents(curDate)
        ...
        if components.day! > 0 {
            ...
            if components.hour! > 23 {
                ...
                return "20小时前"
            } else if components.hour! > 19 {
                return "19小时前"
            } else if components.hour! > 18 {
                return "18小时前"
            } else if components.hour! > 17 {
                return "17小时前"
            } else if components.hour! > 16 {
                return "16小时前"
                ...
                if components.minute! < 1 {
                    ...
                    return components.minute!.toString + "分钟前"
                }
            }
...
extension String {
    func toAgoString() -> String {
        ...
        if let dateTime: Date = dateFormatter.date(from: self) {
            return dateTime.toAgoString()
        }
        return ""
    }
    
    func maxLength(maxLength: Int) -> String {
        if self.length > maxLength{
            return self[0 ..< max(0, maxLength)]
        }
        return self
    }
}
