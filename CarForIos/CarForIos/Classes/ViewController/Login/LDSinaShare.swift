...
class LDSinaShare: NSObject {
    ...
    class func cancelAuth() {
        WeiboSDK.logOut(withToken: UserDefaults.standard.string(forKey: Key_WeiboAccess)!, delegate: LDSinaShare.shared, withTag: "cancel")
    }
}
// MARK: - 分享
            ...
            if let res = response as? WBAuthorizeResponse {
                ...
                if res.statusCode == .success {
                    ...
                    if let uid = res.userID, let access = res.accessToken {
                        ...
                        NotificationCenter.default.post(name: Notification.Name(kWeiboLoginDidSuccess), object: self, userInfo: ["access": access, "uid": uid])
                    ...
                    NotificationCenter.default.post(name: Notification.Name(kWeiboLoginDidFaild), object: self, userInfo: nil)
                } else {
                    if let closure = self.failHandle {
                        closure("授权失败")
                    }
